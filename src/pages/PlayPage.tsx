...
export function PlayPage() {
  ...
  return <>
    ...
    <Section>
      <Reveal className="play-launch"><div className="play-logo"><img src="/mathsprout-logo.jpeg" alt={t('game.logoAlt')} width="140" height="140" /></div><div><div className="device-row"><span><Laptop aria-hidden="true" /> {t('play.desktop')}</span><span><Tablet aria-hidden="true" /> {t('play.tablet')}</span></div><h2>{t('common.play')}</h2><p>{t('play.statusText')}</p><a className="button button-primary" href={liveGameUrl} target="_blank" rel="noreferrer">{t('common.play')} <ExternalLink aria-hidden="true" /><span className="sr-only">{t('a11y.newTab')}</span></a></div></Reveal>
    </Section>
